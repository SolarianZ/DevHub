...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify_only:
        print(f"Python SDK validation ready: {checks_dir}")
    else:
        print(f"Python SDK release assets ready: {result.output_dir}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Python SDK packaging failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
