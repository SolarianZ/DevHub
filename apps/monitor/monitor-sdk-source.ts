...
export const DEFAULT_MONITOR_SDK_SOURCE = "release";
export const LOCAL_MONITOR_SDK_SOURCE = "local-src";

export type MonitorSdkSource = typeof DEFAULT_MONITOR_SDK_SOURCE | typeof LOCAL_MONITOR_SDK_SOURCE;
...
export function readMonitorSdkSource(env = process.env): MonitorSdkSource {
  ...
  if (candidate === DEFAULT_MONITOR_SDK_SOURCE || candidate === LOCAL_MONITOR_SDK_SOURCE) {
    return candidate;
  }

  throw new Error(
    `${MONITOR_SDK_SOURCE_ENV} 必须为 ${DEFAULT_MONITOR_SDK_SOURCE} 或 ${LOCAL_MONITOR_SDK_SOURCE}，实际收到：${candidate}`
  );
}
...
export function resolveMonitorSdkSourceConfig(env = process.env): MonitorSdkSourceConfig {
  ...
  if (source === DEFAULT_MONITOR_SDK_SOURCE) {
    return {
      source,
      isLocalSource: false,
      resolveAlias: [],
