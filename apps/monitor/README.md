...
- `npm test`：执行前端主界面、设置/帮助菜单、定义工作流，以及基于真实 Host fixture 的前端回归。
...
- `src/`：前端 WebView 工程；`App.tsx` 负责单主页状态编排，bootstrap / Host 会话 / Definition 编辑分别落在独立 hooks，设置与帮助能力由壳层菜单统一承载。
...
- Monitor 只有一个阶段驱动的主界面：未发现可用 Host 时展示发现 / 启动流程，发现并验证可用 Host 后自动切换为连接摘要、应用定义和实例清单。
- 顶部菜单栏提供 `设置` 和 `帮助` 两个入口；`设置` 在当前壳层内打开编辑层，`帮助` 提供打开 Host / Monitor 日志目录的动作。
- 设置中的 `DEVHUB_DATA_DIR` 覆盖值和 Host 可执行文件路径都只接受绝对路径；相对路径会在前端与原生命令层同时被拒绝。
...
Monitor 启动后会先扫描当前有效 `DEVHUB_DATA_DIR`，只有在真实 `hub.ping` 校验成功后才切换到运行态摘要。若未配置 Host 可执行文件路径，启动请求会保留当前主界面，并引导用户通过顶部 `设置` 菜单补全配置。

## 日志与能力边界

- Host 日志固定来自 `<DEVHUB_DATA_DIR>/logs/`。
- Monitor 自身结构化日志写入 Monitor 本地数据目录下的 `logs/monitor-YYYYMMDD.jsonl`；当前目录会显示在设置编辑层的“Monitor 日志目录”字段中。
- 前端不再提供内嵌日志列表或日志内容预览；需要排障时，通过顶部 `帮助` 菜单直接调用系统外壳打开 Host / Monitor 日志目录。
