...
class TestConformanceRunner(unittest.TestCase):
    ...
    def test_restart_suite_host_should_reload_current_runtime_snapshot(self) -> None:
        with tempfile.TemporaryDirectory(prefix="devhub-conformance-restart-") as temp_root_str:
            temp_root = Path(temp_root_str)
            host_context, process, log_file = vector_setup.start_suite_host(temp_root)

            try:
                first_pid = host_context.hub_info["pid"]

                restarted_context, restarted_process = vector_setup.restart_suite_host(
                    host_context,
                    process,
                    log_file,
                )
                ...
                self.assertEqual(restarted_process.pid, restarted_context.hub_info["pid"])
                self.assertNotEqual(first_pid, restarted_context.hub_info["pid"])
            finally:
                vector_setup.stop_process(process)
                log_file.close()
    ...
    def test_CONF_005_vector_runner_should_support_case_id_filter(self) -> None:
        with tempfile.TemporaryDirectory(prefix="devhub-conformance-case-id-") as temp_root_str:
            ...
            self.assertIn("PASS  CONF-001 discovery.invalid_httpbaseurl_path_rejected", completed.stdout)
            self.assertIn("SUMMARY  total=11 passed=11 failed=0", completed.stdout)
