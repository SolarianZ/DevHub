...
本仓库包含 DevHub 协议规范、规划文档、实现代码与测试。所有实现与测试必须以 `docs/Spec.md` 为准；若与其他文档或本文件冲突，以 `docs/Spec.md` 为最终权威。
...
- JSON-RPC 2.0；HTTP `POST /rpc`；**HTTP 始终返回 200**，错误通过 JSON-RPC `error` 表达。
- **不支持 batch**；数组根请求必须返回 `invalid_request`。
- `hub.*` 成功结果必须是对象，且至少包含 `{ "ok": true }`。
- 必须携带 HTTP 头：`Authorization: Bearer <token>`、`X-DevHub-Protocol: 1`、`X-DevHub-ClientId`、`X-DevHub-ClientSessionId`。
- `scope` 规则：`null/omitted` 为 global；空字符串非法；字符串 `"global"` 禁止；**指定 scope 时绝不回落到 global**。
- `AppDefinition.scopePolicy` 必须严格执行（`any`/`globalOnly`/`required`）。
- 仅监听 loopback（`127.0.0.1`/`localhost`/`::1`）；跨用户访问不支持。
- WebSocket 必须先 `hub.ws.authenticate` 才能进行其他调用。

> 详细字段、错误码、状态机与时序要求请参考 `docs/Spec.md`。

## 技术栈与工程约束（规划）

- 语言与框架：C# + ASP.NET Core（.NET 10）
- JSON：`System.Text.Json`
- 解决方案：仅使用 `.slnx`，不使用旧版 `.sln`
- 规划项目结构：`DevHub.Host` / `DevHub.Core` / `DevHub.Tests`（可按进度调整）
...
- 若发现与 `docs/Spec.md` 冲突，立即以 `Spec.md` 为准并同步本文件。
