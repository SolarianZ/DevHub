...
- 项目尚未正式发布，M6 允许做必要的破坏性调整；但任何调整都必须以 `docs/Spec.md` 为准，禁止通过修改 Spec 迁就实现。
...
- 允许调整：
  ...
  - 文档分类、命名和归档方式。
- 默认不调整：
  - `docs/Spec.md` 已定义的协议字段、错误语义、状态机和序列化契约。
  - 已经由 `Spec.md` 固化的对外协议行为。
- 若为解决根因必须触及公开行为，必须先确认该行为是否属于 `Spec.md` 明确定义；若属于规范行为，则应回到实现或测试修正，而不是改 Spec。
...
- 默认 `docs/Spec.md` 仍是唯一权威标准，任何实现与测试都必须向它对齐。
