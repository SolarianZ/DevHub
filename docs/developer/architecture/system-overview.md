...
本文档说明 DevHub 的模块边界、分层职责、测试组织和长期演进原则。

- 协议字段、错误语义、状态转换、序列化契约与测试断言请查阅 [`Specification.md`](../../specification/protocol/Specification.md)。
- 面向使用者的稳定接入路径请查阅 [`docs/user/README.md`](../../user/README.md)。
- 开发、运维、发布入口请查阅 [`docs/developer/README.md`](../README.md) 与 [`docs/README.md`](../../README.md)。
...
设计目标是在本机闭环内维持清晰边界、稳定契约和可验证行为。跨机器协调、强一致持久队列、复杂权限系统与分布式锁不属于当前架构范围。

## 2. 权威边界

| 主题 | 权威入口 | 说明 |
| --- | --- | --- |
| 协议契约 | [`Specification.md`](../../specification/protocol/Specification.md) | 定义公开字段、状态机、错误码、方法语义与版本化资产 |
| 使用路径 | [`docs/user/README.md`](../../user/README.md) | 面向 Host、SDK 与原始协议接入方的当前有效做法 |
| 开发维护 | [`docs/developer/README.md`](../README.md) | 面向仓库开发者的架构、开发、运维与发布文档 |
| 测试入口 | [`host/tests/README.md`](../../../host/tests/README.md) | 说明仓库级测试分层、执行方式与 conformance 自测入口 |
...
适配层直接面向公开协议，因此必须严格对齐 `Specification.md`。任何协议层错误处理、参数默认值、错误码映射和序列化契约都应在这一层显式实现，而不是下沉到 `DevHub.Core`。
...
- `docs/user/`：如何使用 DevHub
- `docs/developer/`：如何开发、维护、运维和发布 DevHub
- `docs/specification/`：协议事实、Schema 与原始协议示例
- `docs/assets/`：静态资源

这样做的目的不是增加文档数量，而是避免多个文档同时维护同一主题的并行副本。外部导航发生变化时，同步更新仓库根 `README.md`、`docs/README.md` 与相关工作区 README。

## 8. 长期演进原则

- 保持 `Specification.md` 单一权威，避免用实现细节反推协议。
- 优先解决跨层耦合和职责混杂的根因，不用临时补丁掩盖结构问题。
- 在未正式发布的稳定公共面上允许做必要收敛，但必须同步更新测试和文档。
- 若后续需要新的阶段任务或治理议题，应通过新的 change 或专题文档表达，不把阶段状态长期保留在架构文档中。
