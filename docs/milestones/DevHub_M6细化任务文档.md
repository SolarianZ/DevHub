...
- [x] 先完成 M6 前置治理校准，明确 `Spec` 中哪些约束属于“核心目标必需”，哪些属于“发布前可调整”，并把该判断标准固定为后续任务的前置决策规则。
- [x] 完成对 `host/`、`sdks/`、测试工程和文档目录的系统性盘点，明确“当前布局是否合理”与“目标布局应是什么”。
- [x] 对不合理的目录放置、工程归属、命名与分层进行调整，确保生产代码、白盒测试、黑盒测试、协议资料和用户文档边界清晰。
- [x] 审查 Host 与三套 SDK 的架构设计，识别并修复职责混杂、依赖方向不清、重复抽象、过度耦合和难以扩展的问题。
- [x] 审查并修正单元测试、集成测试与 conformance 资产对 `Spec.md` 的符合性，清理不必要依赖具体实现细节的测试。
- [x] 完成文档体系整理，使“规范、架构、开发、接入、运维、测试治理”各自有稳定落点，并移除过期或重复的里程碑细化文档。
...
- 允许调整：
  ...
  - 文档分类、命名和归档方式。
- 默认不调整：
  - `docs/spec/Spec.md` 中直接定义协议字段、错误语义、状态机、安全边界和序列化契约的核心行为。
- 允许在 M6 内重审并修正：
  - `Spec` 的冻结状态表述。
...
- `docs/` 当前已按 `spec/`、`architecture/`、`milestones/`、`guides/`、`operations/`、`assets/` 分类组织；其中 `spec/` 统一承载 `Spec.md`、`schema/` 与 `protocol-examples/` 等版本化协议资产。
...
- [x] `M6-GOV-001`：盘点 `Spec`、规划、README、开发/接入/Schema/示例文档中的治理口径，识别把“当前公开基线”误写成“永久冻结承诺”的条款。
- [x] `M6-GOV-002`：将 `Spec` 约束按“核心目标必需 / 发布前可调整 / 实现细节或发布策略不应固化”分类，作为后续 M6 任务的统一决策前置。
- [x] `M6-GOV-003`：同步更新 `Spec`、规划、开发、接入与版本化协议资产文档，确保后续任务统一按同一治理口径推进。
...
- [x] `M6-ARCH-HOST-001`：审查 `DevHub.Core` 中领域模型、RPC 处理、事件总线、文件系统访问、运行时路径和启动编排的职责边界。
...
- [x] `M6-ARCH-HOST-004`：对识别出的边界失衡或重复抽象做集中重构，确保调用链清晰且便于后续维护。
...
- [x] `M6-DOC-001`：为“规范、规划、开发、接入、运维、测试治理、版本化协议资产”定义稳定的文档分类与落点规则。
- [x] `M6-DOC-002`：将目录结构、工程边界、测试分层与验证命令同步到开发文档，避免文档与仓库实际布局脱节。
- [x] `M6-DOC-003`：清理旧里程碑文档中的过期引用、悬空链接与失效状态描述。
- [x] `M6-DOC-004`：在 M6 收尾时统一更新开发进度、里程碑状态与发布前注意事项。
...
- [x] 仓库目录布局经审查后保持清晰、可解释，不再存在明显的职责混放或历史遗留堆叠。
- [x] “协议核心契约”和“发布前可调整约束”的边界已在文档中明确，后续 M6 任务不再被过早冻结口径干扰。
- [x] Host 与三套 SDK 的模块边界、依赖方向和扩展点满足“可靠、简洁、易维护、易扩展”的目标。
- [x] 白盒测试、黑盒测试与 conformance 资产均符合各自分层原则，并严格以 `Spec.md` 为准。
- [x] 所有因 M6 调整引入的破坏性变化都已同步反映到测试、文档、CI 与开发说明。
- [x] 不再保留职责重叠的旧里程碑细化文档，当前阶段只有单一有效的 M6 主文档承接后续治理任务。
