...
- 当前仓库主干布局为：`apps/monitor/` 承载官方桌面 Monitor 工作区，`host/DevHub.slnx` 作为 Host 工作区入口，`host/src/` 承载 Host 生产代码，`host/tests/whitebox/` 承载 .NET 白盒测试工程，`host/tests/` 其余目录承载 Python 黑盒/集成/conformance 与验证资产，`sdks/` 下按语言拆分 SDK 工作区，`docs/` 下按 `spec/`、`architecture/`、`milestones/`、`guides/`、`operations/`、`assets/` 分类维护规范、规划、里程碑、指南与运维资料。
...
`apps/monitor/` 作为 M6 内的官方桌面 GUI 工作区，当前本地验收入口为：

```bash
npm --prefix apps/monitor run verify
```

该入口与 CI 中的 `monitor-tests` job 对齐，覆盖：
