...
- `DEVHUB_RUNTIME_DIR`：运行时目录（`hub.json`、`token.txt`）
- `DEVHUB_APPDEFS_DIR`：应用定义目录（`*.json`）
- `DEVHUB_APPINST_DIR`：实例镜像目录
- `DEVHUB_LOG_DIR`：日志目录（`devhub-*.log`）

### 1.2 默认目录约定

- Windows：`%LOCALAPPDATA%/DevHub/runtime/`
- macOS：`~/Library/Application Support/DevHub/runtime/`
- Linux：`$XDG_DATA_HOME/DevHub/runtime/`（未设置时 `~/.local/share/DevHub/runtime/`）

日志默认位于 `<devhubRoot>/logs/`，可由 `DEVHUB_LOG_DIR` 覆盖。
实例镜像目录默认位于 `<devhubRoot>/apps/instances/`，可由 `DEVHUB_APPINST_DIR` 覆盖。
...
def runtime_dir():
    ...
    if platform.system() == "Windows":
        return Path(os.environ["LOCALAPPDATA"]) / "DevHub" / "runtime"
    if platform.system() == "Darwin":
        return Path.home() / "Library" / "Application Support" / "DevHub" / "runtime"
    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        return Path(xdg) / "DevHub" / "runtime"
    return Path.home() / ".local" / "share" / "DevHub" / "runtime"
...
override = os.environ.get("DEVHUB_LOG_DIR")
if override:
    print(Path(override))
...
| 故障 | 症状 | 核查点 | 恢复动作 |
| --- | --- | --- | --- |
| `hub.json` 缺失 | 客户端无法发现地址，连接失败 | Hub 是否启动；`DEVHUB_RUNTIME_DIR` 是否正确 | 重启 Hub；确认运行时目录权限；重新读取 `hub.json` |
...
- 协议兼容边界请参考 `docs/Spec.md` §9.2。  
- 发布、启动与回滚校验请参考 `docs/部署与运行指南.md`。  
