...
- **发布兼容承诺**：定义首次正式对外发布后如何维持 v1.x 兼容的口径。若这些条款在 M6 期间阻碍对核心目标的纠偏，则允许在正式发布前修订，但**必须**同步更新实现、测试、Schema、示例、README 与接入文档。

本规范中的 MUST / MUST NOT 在 M6 期间按以下原则解释：
- 凡直接约束传输、发现、鉴权、作用域路由、方法语义、状态机、错误语义与安全边界的条款，属于“核心目标必需约束”。
- 凡主要约束首次正式对外发布后的冻结策略、兼容承诺、对外资产版本管理或 conformance 门禁口径的条款，属于“发布前可调整约束”。
- 凡仅服务示例组织、资产打包或仓库发布流程的说明，不应被视为需要在 M6 内固化的协议能力。

当前分类结果如下：
- `§1.1` / `§1.3` 的目标与非目标、`§3` 至 `§8` 的协议行为条款，以及 `§11` 的安全边界条款，按“核心目标必需约束”执行。
- `§4.1.2` 中关于 `hub.json` 字段冻结的表述、`§9` 的兼容承诺条款，以及 `§10` 的 conformance 门禁口径，按“发布前可调整约束”处理。
- 附录、示例命令、版本化 Schema / 协议示例的组织说明等内容，默认视为实现细节或发布策略说明，不单独构成 M6 期间需要冻结的协议能力。

当某条约束是否应被冻结存在争议时，以“它是否直接服务本机单用户多工具编排闭环”为判断标准；若不是，则优先视为发布前可调整约束。
...
- 在当前基线下，Host 与客户端 **必须**使用 §5.4 定义的 `hub.json` 架构。`hub.json` 字段集合在 M6 期间属于“发布前可调整约束”；若为纠正核心目标偏差确需变更，**必须**同步更新 §5.4、Schema、协议示例、SDK、测试与接入文档。首次正式对外发布后，再按 §9.2 冻结既有发现字段。
...
本节定义的是**首次正式对外发布后**的 v1.x 兼容承诺。M6 期间，若某条兼容承诺会阻碍对核心目标的纠偏，则允许在未正式发布前修订本节；但一旦修订，**必须**同步更新实现、测试、SDK、Schema、示例与接入文档。
...
### 10.1 要求的测试类别
M6 期间，conformance 与仓库级回归默认把“协议核心契约”作为刚性门禁；对“发布前可调整约束”，当前测试资产**必须**与当前 Spec 文本保持一致，若规范修订则对应向量、断言与接入说明也**必须**同步修订。
...
2. 通过当前仓库中覆盖上述核心契约的 100% 符合性向量；若某向量对应的是“发布前可调整约束”，则该向量在 M6 期间不是独立冻结承诺，但相关断言**必须**与当前 Spec 文本同步收敛。
