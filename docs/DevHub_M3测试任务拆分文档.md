...
- 本轮完成范围：**C# 白盒第一批已完成并通过**；Python 黑盒专项尚未开始。
- M3 仍需在此基础上补齐 **Scope 一致性黑盒闭环 + runner 接入**。
...
### 2.1 文件规划（新增 + 扩展）
- [ ] 新增 `tests/test_scope_routing.py`。
- [ ] 扩展 `tests/test_launch_invocation.py`（dedupe scope 隔离场景）。
...
- [ ] 省略 `instance.scope` 注册实例，断言在 `listInstances(scope=null)` 可见。
- [ ] 显式 `instance.scope=null` 注册实例，断言行为与 omitted 完全一致。

#### `M3-SCOPE-002` register 显式 scope 精确匹配
- [ ] 注册 `scope="workspace-A"` 实例。
- [ ] `listInstances(scope="workspace-A")` 命中。
- [ ] `listInstances(scope="workspace-a")` 不命中。

#### `M3-SCOPE-003` register/list/launch scope=`""`
- [ ] `registerInstance(scope="")` -> `-32602 invalid_params`。
- [ ] `listInstances(scope="")` -> `-32602 invalid_params`。
- [ ] `launch(scope="")` -> `-32602 invalid_params`。

#### `M3-SCOPE-004` register/list/launch scope=`"global"`
- [ ] `registerInstance(scope="global")` -> `-32602 invalid_params`。
- [ ] `listInstances(scope="global")` -> `-32602 invalid_params`。
- [ ] `launch(scope="global")` -> `-32602 invalid_params`。

#### `M3-SCOPE-005` notify/request 默认 Global 路由
- [ ] 同时注册 Global 实例与 scoped 实例。
- [ ] 发 `notify/request` 且 `target.scope` omitted/null。
- [ ] 仅 Global 实例可 poll 到 invocation。

#### `M3-SCOPE-006` notify/request 显式 scope 不回退
- [ ] 仅注册 Global 实例，发 `target.scope="workspace-A"` 调用。
- [ ] 断言不会由 Global 实例消费；按矩阵返回 `instance_not_found` 或进入 pending。

#### `M3-SCOPE-007` target.scope 非法值
- [ ] `target.scope=""` -> `-32602 invalid_params`。
- [ ] `target.scope="global"` -> `-32602 invalid_params`。

#### `M3-SCOPE-008` 大小写敏感匹配
- [ ] 注册 `workspace-A` 与 `workspace-a` 两类实例。
- [ ] 两个 scope 的 invocation 互不消费。

#### `M3-SCOPE-009` target.instanceId 优先
- [ ] 指定 `target.instanceId` 为不可达实例，且同时存在同 appId 其它实例。
- [ ] 断言不回退到其它实例，返回 `target_instance_missing`。
...
- [ ] 构造 scoped invocation 后由 Global 实例 poll，断言 items 为空。
- [ ] 再由匹配 scope 实例 poll，断言可取到 invocation。

### 2.4 `tests/test_launch_invocation.py` 兼容扩展

#### `M3-SCOPE-011` launch dedupe scope 隔离
- [ ] 相同 `appId`，并发 `scope=workspace-A` 与 `scope=workspace-B` launch。
- [ ] 断言 dedupe key 不串扰（不同 scope 不应互判 `already_running`）。
- [ ] 同一 scope 内重复 launch 仍保持 M2 dedupe 行为。

### 2.5 `tests/test_runner.py` 接入任务
- [ ] 新增 `TestScopeRouting` 模块导入与执行入口。
- [ ] 执行顺序建议：`app_instances -> scope_routing -> invocation -> launch_invocation`。
- [ ] `--fast` 跳过长耗时 lease/offline 等场景，但保留核心 scope 断言。
...
### 4.1 Scope 样本集（M3 固化）
- [ ] `null`：Global。
- [ ] `workspace-A`：显式 scope（大小写样本 1）。
- [ ] `workspace-a`：显式 scope（大小写样本 2）。
- [ ] `"   "`：按严格 Spec 视为**合法显式 scope**（不 trim），用于验证“按原值精确匹配”。

### 4.2 实例与资源隔离
- [ ] 每个测试用例使用唯一 `instanceId`（uuid/timestamp）。
- [ ] 每个测试结束执行 unregister，异常路径也要清理。
- [ ] 按 appId 前缀隔离 M2 与 M3 测试数据。
...
### 6.3 可维护性
- [ ] 新增测试按 M3 专用文件分层，不污染 M1/M2 历史用例结构。
- [ ] 公共逻辑复用 `test_base.py` 与既有测试基础设施，避免重复构造器代码。
...
- [ ] 新增 `tests/test_scope_routing.py` 并接入 `tests/test_runner.py`。
- [ ] 扩展 `tests/test_launch_invocation.py` 的 scope dedupe 隔离用例。
...
- [ ] 输出回归报告（`temp/test_results.json` / `temp/test_results.txt` / `temp/test_log.txt`）。
- [ ] 与 `DevHub_M3细化任务文档.md` 用例编号一一对应并完成交叉复核。
