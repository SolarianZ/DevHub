...
- M5：进行中（`.NET SDK` 子范围已完成 runtime discovery、HTTP JSON-RPC、WebSocket events、统一错误模型、`invocation_failed` 结构化错误辅助、规范事件类型常量、依赖注入工厂接入、打包元数据与 SDK 白盒/黑盒测试；JS/TS SDK 与 conformance 资产尚未创建）。
